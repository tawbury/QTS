...
단일 앱 진입점입니다.
이전 main.py의 로직을 새로운 app/ 구조에 맞게 재구성했습니다.

실행 방법:
    python -m app.main --local-only --verbose
    python -m app.main --scope scalp --broker kiwoom
...
# 프로젝트 루트를 sys.path에 추가 (app.*, ops.*, shared.* import 가능하도록)
_ROOT = Path(__file__).resolve().parent.parent  # qts/
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

# app 내부 imports
from app.core.config.config_loader import load_local_only_config, load_unified_config
from app.core.config.config_models import ConfigScope, UnifiedConfig
from app.core.config.config_validator import validate_config, ConfigValidationError
from app.core.config.env_loader import load_dotenv_if_available
from app.monitoring.central_logger import configure_central_logging
from app.pipeline.loop.eteda_loop import run_eteda_loop
...
def _create_mock_runner(
    ...
    Args:
        ...
        observer_client: Observer Client (stub)

    Returns:
        (runner, snapshot_source, should_stop_fn) 튜플
    """
    from app.data.mock_sheets_client import MockSheetsClient
    from app.pipeline.mock_safety_hook import MockSafetyHook
    from app.pipeline.loop.mock_snapshot_source import (
        MockSnapshotSource,
...
def _create_production_runner(
    ...
    Args:
        ...
        observer_client: Observer Client (UDS/IPC)

    Returns:
        (runner, snapshot_source, should_stop_fn) 튜플
    """
    from app.pipeline.eteda_runner import ETEDARunner
    from app.pipeline.loop.eteda_loop_policy import default_should_stop_from_config
    from app.core.config.env_loader import get_broker_config, is_real_order_enabled
    from app.execution.adapters.order_adapter_to_broker_engine_adapter import (
        OrderAdapterToBrokerEngineAdapter,
    )
    from app.execution.brokers.live_broker import LiveBroker
    from app.data.google_sheets_client import GoogleSheetsClient
    from app.pipeline.safety_hook import SafetyHook
    from app.execution.clients.broker.adapters.registry import get_broker_adapter
...
def main() -> int:
    ...
    try:
        if local_only:
            ...
            _LOG.info(f"Observer Client: {observer_type}")
