...
## 4. Observer 확장 기능 연동 (Task 06~08)

### 4.1 성능 모니터링 연동

Observer는 내부 성능 모니터링을 포함하며, 이는 **순수 관찰용**이며 Observer 동작에 영향을 주지 않는다.

**제공되는 성능 지표:**
- 스냅샷 처리 지연 시간 (validation, guard, record creation, enrichment, dispatch)
- 버퍼 깊이 및 플러시 작업 메트릭
- 레코드 처리 카운터 (수신, 처리, 차단)
- 시스템 업타임 및 타이밍 통계

**연동 방식:**
- 내부 컴포넌트는 `performance_metrics.py`를 통해 메트릭 기록
- 외부 시스템은 `performance_monitoring_interface.py`를 통해 읽기 전용 액세스
- 메트릭은 메모리 내에만 저장되며 프로세스 재시작 시 초기화

**주요 제약사항:**
- 성능 메트릭은 Observer 동작에 영향을 주지 않음
- 자동 튜닝이나 의사결정 로직에 사용되지 않음
- 순수 관찰용 데이터만 제공

### 4.2 확장 설정 연동

Observer는 두피 확장 기능을 위한 추가 설정을 지원한다.

**설정 섹션:**
- **하이브리드 트리거**: 활성화/비활성화, 틱 소스 선택, 간격 제한
- **버퍼 관리**: 플러시 간격, 버퍼 크기 제한, 활성화/비활성화
- **로그 로테이션**: 로테이션 활성화, 윈도우 설정, 파일 제한
- **성능 모니터링**: 활성화/비활성화, 히스토리 크기 제한

**연동 방식:**
- 시작 시 `config_manager.py`를 통해 설정 로드
- 내부 컴포넌트는 설정 관리자를 통해 파라미터 접근
- 설정 유효성 검증과 안전한 기본값 제공

**주요 제약사항:**
- 설정은 Observer 책임을 변경하지 않음
- 의사결정 로직이나 전략 파라미터를 포함하지 않음
- 런타임 설정 변경은 없음 (재시작 필요)

### 4.3 테스트 인프라 연동

Observer는 포괄적인 테스트 인프라를 포함한다.

**테스트 범주:**
- **단위 테스트**: 성능 메트릭, 설정 관리, 버퍼 작업
- **통합 테스트**: 모의 파이프라인 컴포넌트를 사용한 종단 간 워크플로우
- **모의 컴포넌트**: 틱 소스, 이벤트 버스, 외부 의존성

**연동 방식:**
- 테스트는 Observer 동작을 변경하지 않고 검증
- 모의 환경과 제어된 조건 사용
- 추가 및 제거 가능한 테스트 구조

**주요 제약사항:**
- 테스트는 Observer 책임에 영향을 주지 않음
- 의사결정 로직 테스트 없음
- 기존 동작 검증용, 수정용 아님
