...
- [ ] 코드 품질 개선(필수)
  - [ ] `PerformanceEngine`의 mock 생성 경로(`_generate_mock_*`) 제거 또는 테스트 전용으로 격리
  - [ ] 랜덤/seed 기반 결과가 런타임 결과에 섞이지 않도록 방지
- [ ] 실제 데이터 경로 강화
  - [ ] History/Performance repository 기반 KPI 산출 흐름 점검

## 완료 조건

- [ ] 런타임 엔진 결과가 “실제 데이터 기반”으로만 생성된다.
- [ ] Mock 로직은 테스트 전용 경로로만 존재한다(또는 제거).
